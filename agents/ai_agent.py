...
async def get_blog_analysis_perplexity(text: str, metrics: dict) -> dict:
    """
    Use Perplexity to generate blog analysis copy based on provided metrics.
    Returns a dict with overview, summary, strengths, weakSpots, improvements, suggestions.
    """
    compact_text = re.sub(r"\s+", " ", text or "").strip()[:12000]
    prompt = f"""
    ...
    try:
        ...
        print(f"Error in blog analysis (Perplexity): {e}")
        return {}
